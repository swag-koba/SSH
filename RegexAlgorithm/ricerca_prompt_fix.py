...
def clean_author_match(matched_text: str) -> str | None:
    ...
    name_tokens = []
    for token in doc:
        emit(f"TOKEN: '{token.text}' | POS: {token.pos_} | TAG: {token.tag_} | ENT: {token.ent_type_}")
        if token.pos_ in {"PROPN"} and not token.is_stop:
...
def analizza_query(text: str, soglia: float = 0.5) -> str:
    ...
    emit(f"Casi rilevati: {formatted or 'nessuno'}")
    ...
    #          f"ma il metodo individuato è {metodo}")ù
